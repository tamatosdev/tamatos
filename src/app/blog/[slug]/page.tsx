...
async function getNextPost(currentSlug: string): Promise<Post | null> {
  try {
    const data = await client.request<{ posts: { nodes: Post[] } }>(nextPostQuery);
    const posts = data.posts.nodes;
    const idx = posts.findIndex((p) => p.slug === currentSlug);
    return posts[idx + 1] ?? posts[0] ?? null;
  } catch {
    return null;
  }
}
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [post, nextPost] = await Promise.all([getPost(slug), getNextPost(slug)]);
    ...
    <>
      ...
      <main className="relative" style={{ overflowX: "clip", zIndex: 1 }}>
        ...
        {nextPost && (() => {
          ...
          const npReadingTime = np.content ? getReadingTime(np.content) : "03 Mins Read";
          const npExcerpt = np.excerpt ? getExcerpt(np.excerpt) : np.content ? getExcerpt(np.content) : "";
